...
import { Search, LayoutDashboard, Activity, CheckCircle2, ImagePlus, Camera, FileText, X, LogIn, LogOut, User, Headset, Zap, Settings2, ChevronDown, Package, Loader2, Ticket, Tag, Copy, AlertTriangle, Stethoscope, ShoppingBag, Plus, Trash2 } from 'lucide-react';
...
export default function Home() {
  ...
  const [autodiagAttachments, setAutodiagAttachments] = useState<{type: 'image'|'pdf'; base64: string; name: string; preview: string|null}[]>([]);
  ...
  const processAutodiagImage = (file: File) => {
    const reader = new FileReader();
    reader.onloadend = () => {
      try {
        const img = new window.Image();
        img.onload = () => {
          ...
          setAutodiagAttachments(prev => {
            if (prev.length >= MAX_ATTACHMENTS) return prev;
            return [...prev, { type: 'image' as const, base64: compressed, name: file.name, preview: compressed }];
          });
        };
        img.src = reader.result as string;
      } catch (_) {}
  ...
  const handleAutodiagCameraCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !file.type.startsWith('image/')) return;
    processAutodiagImage(file);
    e.target.value = '';
  };

  const handleAutodiagGalleryUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    files.forEach(file => { if (file.type.startsWith('image/')) processAutodiagImage(file); });
    e.target.value = '';
  };

  const handleAutodiagPdfUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    files.forEach(file => {
      if (file.type === 'application/pdf') {
        ...
        reader.onloadend = () => {
          setAutodiagAttachments(prev => {
            if (prev.length >= MAX_ATTACHMENTS) return prev;
            return [...prev, { type: 'pdf' as const, base64: reader.result as string, name: file.name, preview: null }];
          });
        };
        reader.readAsDataURL(file);
      }
  ...
  const removeAutodiagAttachment = (idx: number) => {
    setAutodiagAttachments(prev => prev.filter((_, i) => i !== idx));
  };
  ...
  const handleAutodiagSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!autodiagForm.problema.trim() && autodiagAttachments.length === 0) {
      setAutodiagError('Descreva o problema ou envie ao menos uma imagem, foto ou PDF para iniciar o diagnóstico.');
      return;
    }
    if (!user) {
      const anonSearches = parseInt(localStorage.getItem('autoparts_anon_searches') || '0', 10);
    ...
    try {
      ...
      if (autodiagAttachments.length > 0) payload.attachments = autodiagAttachments.map(a => ({ type: a.type, base64: a.base64 }));
          ...
          {activeTab === 'smart' ? (
            ...
            <div className="w-full bg-[#1C1C1E]/70 backdrop-blur-2xl border border-white/10 rounded-[32px] shadow-[0_8px_30px_rgb(48,209,88,0.05)] transition-all duration-300 animate-in fade-in zoom-in-95 overflow-hidden">
              ...
              {autodiagResult ? (
                ...
                <div className="p-6">
                  <button
                    onClick={() => { setAutodiagResult(null); setAutodiagError(null); setAutodiagAttachments([]); }}
                ...
                <form onSubmit={handleAutodiagSubmit} className="p-6 space-y-5">
                  ...
                  {/* Arquivos: Câmera / Galeria / PDF */}
                  <div>
                    <h3 className="text-[13px] font-bold text-[#8E8E93] uppercase tracking-widest mb-3">5. Fotos, Tela do Scanner ou Relatório PDF (Opcional · máx. 5)</h3>
                    {/* Preview grid dos arquivos adicionados */}
                    {autodiagAttachments.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-3">
                        {autodiagAttachments.map((att, idx) => (
                          <div key={idx} className="relative group">
                            {att.type === 'image' ? (
                              <img src={att.preview!} alt={att.name} className="h-20 w-20 object-cover rounded-2xl border border-white/10" />
                            ) : (
                              <div className="h-20 w-20 bg-[#2C2C2E]/60 border border-white/10 rounded-2xl flex flex-col items-center justify-center gap-1 px-1">
                                <FileText className="w-7 h-7 text-[#FF9500]" />
                                <span className="text-[9px] text-[#8E8E93] text-center w-full truncate px-1">{att.name.length > 10 ? att.name.slice(0, 9) + '…' : att.name}</span>
                              </div>
                            )}
                            <button type="button" onClick={() => removeAutodiagAttachment(idx)}
                              className="absolute -top-1.5 -right-1.5 bg-[#2C2C2E] border border-white/10 text-gray-200 rounded-full p-1 opacity-0 group-hover:opacity-100 transition-all hover:bg-[#FF3B30]/20">
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                        ))}
                    ...
                    {/* Botões de upload */}
                    {autodiagAttachments.length < MAX_ATTACHMENTS && (
                      <div className="grid grid-cols-3 gap-2">
                        {/* Câmera */}
                        <label className="flex flex-col items-center gap-1.5 cursor-pointer bg-[#2C2C2E]/40 border border-dashed border-white/20 hover:border-[#30D158]/40 rounded-2xl py-3.5 transition-colors group">
                          <input type="file" accept="image/*" capture="environment" className="hidden" onChange={handleAutodiagCameraCapture} />
                          <Camera className="w-5 h-5 text-[#8E8E93] group-hover:text-[#30D158] transition-colors" />
                          <span className="text-[11px] text-[#8E8E93] group-hover:text-white transition-colors">Câmera</span>
                        </label>
                        {/* Galeria */}
                        <label className="flex flex-col items-center gap-1.5 cursor-pointer bg-[#2C2C2E]/40 border border-dashed border-white/20 hover:border-[#30D158]/40 rounded-2xl py-3.5 transition-colors group">
                          <input type="file" accept="image/*" multiple className="hidden" onChange={handleAutodiagGalleryUpload} />
                          <ImagePlus className="w-5 h-5 text-[#8E8E93] group-hover:text-[#30D158] transition-colors" />
                          <span className="text-[11px] text-[#8E8E93] group-hover:text-white transition-colors">Galeria</span>
                        </label>
                        {/* PDF */}
                        <label className="flex flex-col items-center gap-1.5 cursor-pointer bg-[#2C2C2E]/40 border border-dashed border-white/20 hover:border-[#30D158]/40 rounded-2xl py-3.5 transition-colors group">
                          <input type="file" accept="application/pdf" multiple className="hidden" onChange={handleAutodiagPdfUpload} />
                          <FileText className="w-5 h-5 text-[#8E8E93] group-hover:text-[#FF9500] transition-colors" />
                          <span className="text-[11px] text-[#8E8E93] group-hover:text-white transition-colors">PDF</span>
                        </label>
                      </div>
                    )}
                    {autodiagAttachments.length > 0 && (
                      <p className="text-[11px] text-[#8E8E93] mt-2">{autodiagAttachments.length}/{MAX_ATTACHMENTS} arquivo(s) • Passe o mouse para remover</p>
                    )}
                  </div>
